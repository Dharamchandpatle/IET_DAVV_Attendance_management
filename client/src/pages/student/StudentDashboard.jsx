...
export default function StudentDashboard() {
	...
	useEffect(() => {
		...
		const refreshData = async () => {
			if (!user?.id) return;
			try {
				// student basic info from auth context
				const data = {
					id: user.id,
					name: user.name,
					email: user.email,
				...
				if (mounted) setStudentData(data);
				...
				try {
					...
					const filtered = Array.isArray(records) ? records.filter(r => Number(r.semester) === Number(selectedSemester)) : [];
					...
					if (mounted) setSubjectAttendance([
				...
				// Fetch leave requests count for this student
				try {
					const leaves = await listLeaveRequests();
					if (mounted) setLeaveCount(Array.isArray(leaves) ? leaves.length : 0);
				} catch (leaveErr) {
					console.warn('Failed to load leave requests count', leaveErr);
				}
			} catch (error) {
				console.warn('Failed to refresh student dashboard data', error);
			} finally {
	...
	const studentKPIs = [
		...
		{ title: 'Leave Requests', value: `${leaveCount}`, icon: FileText }
	];
